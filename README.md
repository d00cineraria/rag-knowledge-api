...
### リランカーA/B実測（2026-08-08, retrieval-only, 44問, 同一コレクション）

「正解は上位8件に入るが順位が甘い」というPhase 2の診断に対し、bge-reranker-v2-m3（CPU）を有効化して同一コレクション・同一質問で比較:

| 指標 | リランカーOFF | リランカーON | 差分 |
|---|---|---|---|
| recall@3 | 0.946 | **1.000** | +0.054 |
| recall@8 | 1.000 | 1.000 | ±0.000 |
| MRR | 0.865 | **0.919** | +0.054 |
| nDCG@8 | 0.887 | **0.938** | +0.050 |

診断通りリランカーが順位付けを改善し、全指標が向上（recall@3は満点に回復）。代償はレイテンシ（CPUクロスエンコーダで1問あたり数秒）で、品質と速度のトレードオフとして`RERANKER_ENABLED`で切り替え可能にしている。
